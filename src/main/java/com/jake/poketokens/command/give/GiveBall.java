...
public class GiveBall implements CommandExecutor {
    ...
    public CommandResult execute(@Nonnull CommandSource src, @Nonnull CommandContext args) throws CommandException {
        ...
        ItemStack token = Tokens.ballToken(amount);
        ...
        if(ItemUtil.giveItem(player, token)){
            player.sendMessage(deserialize("&2*&a&oYou've been given a Ball Token&2*"));
        } else {
            player.sendMessage(deserialize("&2*&a&oThe Ball Token would not fit in your inventory, so it was dropped at your feet&2*"));
        }
        PokeTokens.logger.info(player.getName() + " has been given a Ball Token");
