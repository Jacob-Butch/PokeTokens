...
public class GiveHA implements CommandExecutor {
    ...
    public CommandResult execute(@Nonnull CommandSource src, @Nonnull CommandContext args) throws CommandException {
        ...
        ItemStack token = Tokens.haToken(amount);
        ...
        if(ItemUtil.giveItem(player, token)){
            player.sendMessage(deserialize("&2*&a&oYou've been given a Hidden Ability Token&2*"));
        } else {
            player.sendMessage(deserialize("&2*&a&oThe Hidden Ability Token would not fit in your inventory, so it was dropped at your feet&2*"));
        }
        PokeTokens.logger.info(player.getName() + " has been given a Hidden Ability Token");
